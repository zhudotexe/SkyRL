...
When `generator.use_cache_salt=true` (the default), `HarborGenerator` computes a per-policy-version salt once per `generate` call — keyed on the inference engine's weight version and served model name — and injects it into each trial's config as `agent.kwargs.llm_kwargs.extra_body.cache_salt`. Harbor's `LiteLLM` wrapper forwards `extra_body` to `litellm.acompletion()`, which passes `cache_salt` as a top-level field of the vLLM chat-completions request. vLLM mixes the salt into its prefix-cache block hashes so blocks are only shared between trajectories started at the same policy version — cache is reused within a version (throughput) and isolated across weight updates (correctness). This matters for fully-async training, where the prefix cache is deliberately not fully cleared on every weight sync. See [`generator.use_cache_salt`](/docs/configuration/config) for details.
