...
This recipe uses a **different server command** than the common one at the top of this page, in particular the `remove_microbatch_padding` override is VLM-specific:

```bash
uv run --isolated --extra tinker --extra fsdp \
    -m skyrl.tinker.api \
    --base-model "Qwen/Qwen3-VL-8B-Instruct" --backend fsdp \
    --backend-config '{"trainer.remove_microbatch_padding": false}'
```
