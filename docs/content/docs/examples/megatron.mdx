...
For details on configuring the Megatron backend, and enabling checkpointing, see the [configuration API reference](/docs/api-ref/skyrl/config) and the [Megatron checkpointing guide](../checkpointing-logging/checkpointing#megatron-checkpointing).
...
For more details on configuring the Megatron backend, and enabling checkpointing, see the [configuration API reference](/docs/api-ref/skyrl/config), and the [Megatron checkpointing guide](../checkpointing-logging/checkpointing#megatron-checkpointing).
...
for advanced users to fully take advantage of all of Megatron-Core's feature flags. For the full field list, defaults, and per-field documentation, see [`MegatronConfig`](/docs/api-ref/skyrl/config) in the configuration API reference.
...
### Sizing the parallelism dimensions

The parallel sizes must satisfy the following rules:

- `model_size = pp_size * tp_size * cp_size`
- `dp_size = world_size / model_size`
- `world_size % (pp_size * ep_size * etp_size) == 0`

The last rule means that `ep_size * etp_size` can scale independently of `tp_size * cp_size`, and
can go across data parallel ranks.

Two things worth noting when picking these values:

- Megatron sequence parallelism (unrelated to Ulysses `sequence_parallel_size`) is enabled
  automatically whenever `tensor_model_parallel_size > 1`.
- `expert_tensor_parallel_size` resolves to `tensor_model_parallel_size` when left as `null`.
  Setting it to `1` is recommended for best performance when `expert_model_parallel_size > 1`.
...
Accepted dtype names are case-insensitive: `fp32` (`float32`, `float`), `fp16` (`float16`, `half`),
`bf16` (`bfloat16`), and `fp8` (`float8`, `uint8`). `fp8` maps to `torch.uint8`, matching
TransformerEngine optimizer state storage.
...
<Callout type="warn">
`optimizer_config_kwargs.use_precision_aware_optimizer=true` can cause checkpointing to fail. See:
https://github.com/nvidia/megatron-lm/issues/1820. We recommend leaving this setting to `false`.
</Callout>
