...
## Prerequisite: Use the SkyRL HTTP Inference Endpoint

SkyRL exposes the inference engine as an OpenAI-compatible HTTP endpoint, so your agent harness can send requests to it. Generators obtain the endpoint URL from the inference engine client via `get_endpoint_url()` rather than a fixed host/port.
